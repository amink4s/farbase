...
export const metadata = { title: 'Explore — Tokens' };
...
export default async function Page() {
  ...
  // Filter articles where metadata->>'category' = 'token'
  const url = `${SUPABASE_URL}/rest/v1/articles?select=slug,title,metadata,author_fid,created_at&metadata->>category=eq.token&order=created_at.desc&limit=200`;
  const resp = await fetch(url, { headers: { Authorization: `Bearer ${SUPABASE_KEY}`, apikey: SUPABASE_KEY }, next: { revalidate: 60 } });
  if (!resp.ok) {
    return <div style={{ padding: 24 }}>Failed to load tokens list</div>;
  }
